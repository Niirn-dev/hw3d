...
{
	const auto selectedID = (pSelectedNode != nullptr) ? pSelectedNode->GetID() : -1;
...
std::unique_ptr<Mesh> Model::ParseMesh( Graphics& gfx,const aiMesh& mesh,const aiMaterial* const* pMaterial )
{
	...
	auto pvsbc = std::static_pointer_cast<Bind::VertexShader>( pvs )->GetBytecode();
...
std::unique_ptr<Node> Model::ParseNode( int& id,const aiNode& node ) noexcept
{
	namespace dx = DirectX;
	...
	auto pNode = std::make_unique<Node>( id++,node.mName.C_Str(),std::move( curMeshPtrs ),transform );
