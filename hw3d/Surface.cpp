...
#define FULL_WINTARD
#include "Surface.h"
// the workaournd so gdi+ won't complain about not having min max macros
#include <algorithm>
namespace Gdiplus
{
	using std::min;
...

	{
		...
		for ( unsigned int y = 0; y < height; y++ )
		{
			for ( unsigned int x = 0; x < width; x++ )
			{
				...
				pBuffer[(size_t)y * width + x] = Color( c.GetA(),c.GetB(),c.GetG(),c.GetR() ); ///< Colors are read in a weird order
			}
		}
	}
